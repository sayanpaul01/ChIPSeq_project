...
## **📌 CTCF peaks cutoff (before deduplication)**
...
## **📌 CTCF peaks length cutoff (before deduplication)**
```{r CTCF_length, echo=TRUE, fig.width=16, fig.height=10}
library(tidyverse)
library(readr)
library(scales)   # for label_number()
...
p_log <- df %>%
  mutate(lpeaks = ifelse(lpeaks <= 0, NA_real_, lpeaks)) %>%
...
## **📌 RAD21 peaks length cutoff (before deduplication)**
```{r RAD21_length, echo=TRUE, fig.width=16, fig.height=10}
library(tidyverse)
library(readr)
library(scales)   # for label_number()
...
p_linear <- ggplot(df, aes(qscore, lpeaks, group = Sample)) +
  ...
  x_ticks + coord_cartesian(xlim = c(0, 7)) +
  scale_y_continuous(labels = label_number(big.mark = ",")) +
  labs(title = "RAD21 narrow peaks length across samples (before deduplication)",
       x = "Q Score", y = "Total Peak Length (bp, linear)") +
  theme_minimal(base_size = 12) +
  theme(strip.text = element_text(face = "bold", size = 10),
        panel.grid.minor = element_blank(),
        plot.title = element_text(hjust = 0.5, face = "bold"))
...
p_log <- df |>
  ...
  scale_y_log10(labels = label_number(big.mark = ",")) +
  labs(title = "RAD21 narrow peaks length across samples (before deduplication)",
       x = "Q Score", y = "Total Peak Length (bp, log10)") +
  theme_minimal(base_size = 12) +
  theme(strip.text = element_text(face = "bold", size = 10),
        panel.grid.minor = element_blank(),
        plot.title = element_text(hjust = 0.5, face = "bold"))
...
## **📌 CTCF peaks length cutoff (after deduplication)**
```{r CTCF_dedup_length, echo=TRUE, fig.width=16, fig.height=10}
library(tidyverse)
library(readr)
library(scales)   # for label_number()
...
## **📌 RAD21 peaks length cutoff (after deduplication)**
```{r RAD21_dedup_length, echo=TRUE, fig.width=16, fig.height=10}
library(tidyverse)
library(readr)
library(scales)   # for label_number()
...
p_linear <- ggplot(df, aes(qscore, lpeaks, group = Sample)) +
  ...
  x_ticks + coord_cartesian(xlim = c(0, 7)) +
  scale_y_continuous(labels = label_number(big.mark = ",")) +
  labs(title = "RAD21 narrow peaks length across samples (after deduplication)",
       x = "Q Score", y = "Total Peak Length (bp, linear)") +
  theme_minimal(base_size = 12) +
  theme(strip.text = element_text(face = "bold", size = 10),
        panel.grid.minor = element_blank(),
