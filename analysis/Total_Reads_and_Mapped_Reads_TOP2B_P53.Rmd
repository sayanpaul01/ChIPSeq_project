...
long5 <- map %>%
  ...
  droplevels() %>%
  mutate(Reads = ifelse(Reads < 0, 0, Reads))  # clamp negatives, ensure baseline 0
...
ggplot(long5, aes(x = Metric, y = Reads)) +
  ...
  scale_y_continuous(
    ...
    expand = c(0, 0)  # removes bottom padding
  ) +
  coord_cartesian(ylim = c(0, NA)) +  # forces y-axis start from 0
